...
public:
    ...
    bool insert(int key)
    {
        ...
        while (i < m && table[index].status != EMPTY)
        {
            ...
            if (table[index].status == DELETED && first_index_deleted == -1)
            {
                first_index_deleted = index;
            }
        ...
        if (first_index_deleted != -1)
        {
            table[first_index_deleted].val = key;
            table[first_index_deleted].status = OCCUPIED;
